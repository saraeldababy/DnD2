...
Game::SaveState Game::getSaveState() const
{
    ...
    // Witch state
    s.witchWrongAttempts = witchScene ? witchScene->wrongAttempts() : 0;
    s.witchEscaped       = witchScene ? witchScene->escaped()       : false;

    // If saved mid-level-3 after gargoyles, skip back to corridor not splash
    if (s.currentLevel == 3) {
        Phase p = (Phase)s.phaseInt;
        if (p == Phase::LEVEL3_CORRIDOR || p == Phase::LEVEL3_POEM) {
            // Keep as-is — loadSaveState will rebuild corridor
        } else if (p == Phase::LEVEL3_GARGOYLE) {
            // Mid-gargoyle — keep, gargoyle HPs are saved
        } else if (p == Phase::LEVEL3_SPLASH || p == Phase::LEVEL3_BRIEFING) {
            // Haven't started yet — fine
        }
    }
    ...
    {
        s.currentLevel = 3;
        s.phaseInt     = (int)Phase::LEVEL3_SPLASH;
    }
    if (leverGateOpen && s.currentLevel == 3 &&
        phase == Phase::LEVEL3_CORRIDOR &&
        level.map[player.getX()][player.getY()] == Level::DOOR)
    {
        s.currentLevel = 4;
        s.phaseInt     = (int)Phase::LEVEL4_NAVIGATE;
    }
    if (cellUnlocked && s.currentLevel == 5 &&
        level.map[player.getX()][player.getY()] == Level::CELL)
    {
        s.currentLevel = 5; // level 5 is final — stay there
    }

    // Lever state
    s.leverProgress = leverProgress;
    s.leverStrikes  = leverStrikes;
    s.leverGateOpen = leverGateOpen;
    for (int i = 0; i < 4; ++i) s.leverLocked[i] = leverLocked[i];

    // Gargoyles
    for (auto &g : gargoyles)
        s.gargoyleHP.append(g.alive ? g.hp : -1);

    // Level 5
    s.hasSword       = hasSword;
    s.dragonDefeated = dragonDefeated;
    s.alarmActive    = alarmActive;
    s.guardsChasing  = guardsChasing;
...
void Game::loadSaveState(const SaveState &s)
{
    ...
    Phase savedPhase = (Phase)s.phaseInt;
    ...
    player.setScore(s.playerScore);
    player.setX(s.playerX);
    player.setY(s.playerY);
    player.takeDamage(player.getHealth());
    player.heal(qMin(s.playerHealth, player.getMaxHealth()));
    for (int i = 0; i < s.playerPotions; ++i) player.addPotion();
    for (int i = 0; i < s.playerKeys;    ++i) player.addKey();
    ...
    int numEnemiesToRestore = qMin((int)s.enemyHP.size(), (int)enemies.size());
    for (int i = 0; i < numEnemiesToRestore; ++i)
        if (s.enemyHP[i] == 0) enemies[i]->defeat();

    if (witchScene && s.witchEscaped)
        witchScene->forceEscaped();
    else if (witchScene)
        for (int i = 0; i < s.witchWrongAttempts; ++i)
            witchScene->submitAnswer("__wrong__");

    leverProgress = s.leverProgress;
    leverStrikes  = s.leverStrikes;
    leverGateOpen = s.leverGateOpen;
    for (int i = 0; i < 4; ++i) leverLocked[i] = s.leverLocked[i];
    if (leverGateOpen) level.map[12][3] = Level::DOOR;

    int numGargoylesToRestore = qMin((int)s.gargoyleHP.size(), (int)gargoyles.size());
    for (int i = 0; i < numGargoylesToRestore; ++i)
    {
        if (s.gargoyleHP[i] == -1) gargoyles[i].alive = false;
        else gargoyles[i].hp = s.gargoyleHP[i];
    }

    hasSword        = s.hasSword;
    dragonDefeated  = s.dragonDefeated;
    alarmActive     = s.alarmActive;
    guardsChasing   = s.guardsChasing;
    if (guardsChasing) activateGuardChase();

    for (int i = 0; i < 4 && i < (int)lockedDoorPositions.size(); ++i)
        if (doorsOpened[i])
            level.map[lockedDoorPositions[i].x()][lockedDoorPositions[i].y()] = Level::DOOR;

    if (cellUnlocked) level.map[11][6] = Level::DOOR;
    if (hasSword)     level.map[7][12] = Level::FLOOR;
}
